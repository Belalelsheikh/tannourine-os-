...
export default function RoutesBuilder() {
  ...
  const save = async () => {
    ...
    // One RPC, one transaction. Saving a day is delete-then-insert, and splitting that
    // across two client calls meant a failed insert left the day empty with its seq values
    // gone. save_route_day also owns the seq rules: existing outlets keep their seq,
    // additions take max+1 in array order, an all-null day stays null.
    const res = await sb.rpc('save_route_day', {
      p_coordinator: coordId,
      p_weekday: day,
      p_outlets: [...selected],
    });
